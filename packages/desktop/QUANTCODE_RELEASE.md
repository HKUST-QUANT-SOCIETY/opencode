...
Linux packages do not require platform code signing. The release job installs `fakeroot` and `rpm`, validates the AppImage (and its blockmap), `.deb`, and `.rpm` names and non-empty contents, and includes all of them in `SHA256SUMS`. `latest-linux.yml` carries the AppImage updater entry; `.deb` and `.rpm` are published as manual-install assets. Distribution repositories may add their own package signatures.
...
The release job validates the exact fourteen installer/package/blockmap outputs before making any GitHub Release visible: eight macOS files, two Windows files, and four Linux files. It merges the two macOS `latest-mac.yml` files, passes through `latest-linux.yml`, verifies the updater entries (AppImage, ZIP, DMG, and EXE) against the local file size and SHA-512, separately validates the DEB/RPM assets, and creates `SHA256SUMS` covering every package. It then creates or reuses a draft release, uploads every asset, compares the remote names, sizes, and GitHub SHA-256 digests, and publishes the draft only after all checks pass. A rerun may replace assets in an existing draft, but it refuses to overwrite an already published release. The exact Linux filenames are `quantcode-<version>-linux-x86_64.AppImage`, `quantcode-<version>-linux-x86_64.AppImage.blockmap`, `quantcode-<version>-linux-amd64.deb`, and `quantcode-<version>-linux-x86_64.rpm`.
