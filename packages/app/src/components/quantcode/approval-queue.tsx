...
export function ApprovalQueue(props: {
  ...
  decide: (threadId: string, checkpointId: string, gateId: string, decision: "approve" | "reject") => void
}) {
  const [state, setState] = createStore({ items: [] as Item[], loading: false, error: "", sent: [] as string[], cursor: undefined as string | undefined })
  let revision = 0
  onCleanup(() => revision++)
  ...
  createEffect(() => { props.scope; revision++; setState("items", []); void load() })
  return <section class="qc-detail-body" aria-label="同组审批队列">
    <h3>同组待审批任务</h3><p>批准或拒绝仅针对当前展示的 Gate；任务变化后需要重新读取。</p>
    <button type="button" disabled={state.loading} onClick={() => void load()}>刷新审批队列</button>
    ...
    <For each={state.items}>{item => <article class="qc-detail-section">
      ...
      <For each={["approve", "reject"] as const}>{decision => <button type="button" disabled={state.loading || state.sent.includes(item.gate.gate_id)} onClick={() => {
        setState("sent", previous => [...previous, item.gate.gate_id])
        props.decide(item.thread_id, item.checkpoint_id, item.gate.gate_id, decision)
      }}>{decision === "approve" ? "批准" : "拒绝"}</button>}</For>
      <Show when={state.sent.includes(item.gate.gate_id)}><p role="status">已请求处理，请查看任务返回结果。</p></Show>
    </article>}</For>
    <Show when={state.cursor}><button type="button" disabled={state.loading} onClick={() => void load(true)}>加载更多待审批任务</button></Show>
  </section>
}
